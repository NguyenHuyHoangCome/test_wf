...
function synchronize_objects_and_verify_response ($device_id, $connection_timeout = 300, $max_time = 300) {
	...
	foreach ($response['wo_newparams'] as $object) {
	    ...
	    $wo_newparams = array_merge($wo_newparams, $response['wo_newparams']);
	}
	$response = prepare_json_response(ENDED, "Objects Synchronization successful for the device $device_id", $wo_newparams, true);
	return $response;
}
