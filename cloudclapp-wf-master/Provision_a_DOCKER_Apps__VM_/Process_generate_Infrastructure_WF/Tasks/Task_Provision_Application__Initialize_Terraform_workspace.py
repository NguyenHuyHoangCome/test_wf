...
if __name__ == "__main__":
    ...
    tf_device_id = 'ClO151'
    context.update(tf_device_id=tf_device_id)
    ...
    terraform_provision_docker_app_workspace = context.get('terraform_provision_docker_app_workspace')
    
    data = dict(device_id=tf_device_id, configuration_file=terraform_provision_docker_app_workspace)
    ...
    if isinstance(data, dict):
        #Orchestration.execute_service_by_reference(subtenant_ext_ref, service_ext_ref, SERVICE_NAME, INIT_PROCESS_NAME, data)
        Orchestration.execute_launch_process_instance(tfw_service_instance_id, INIT_PROCESS_NAME, data)
    ...
    ret = MSA_API.task_success('The Application Terraform Init is done successfully.', context)