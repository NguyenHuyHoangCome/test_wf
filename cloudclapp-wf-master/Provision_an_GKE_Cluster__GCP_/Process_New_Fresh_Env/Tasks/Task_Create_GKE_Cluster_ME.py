...
if __name__ == "__main__":
    ...
    cluster_endpoint = get_tf_output_value(work_directory, 'kubernetes_cluster_host', Orchestration)
    ...
    context.update(cluster_ip=management_address)
    #Kubernetes adaptor does not use the password and login of ME.
    password = 'fake38passwOrd'
    management_port='443'
    #Create Device
    device = Device(customer_id=customer_id, name=context.get('cluster_name'), manufacturer_id=manufacturer_id, model_id=model_id, login=context.get('cluster_name'), password='dummy', password_admin='dummy', management_address=management_address, management_port=management_port, device_external="", log_enabled=False, log_more_enabled=False, mail_alerting=False, reporting=False, snmp_community='ubiqube', device_id="")
    ...
    context.update(gcp_cluster_me_ext_ref=device_ext_ref)
    
    #get device external reference
    device_id = response.get('id')
    context.update(gcp_cluster_me_id=device_id)
    ...
    util.log_to_process_file(context['SERVICEINSTANCEID'], work_directory, context['PROCESSINSTANCEID'])
    ...
    device.create_configuration_variable('cluster_name', context.get('cluster_name'))
