...
def form_app_file(work_directory, app_name, app_image, app_port, app_replicas, app_node_port):
    ...
    with open(app_file) as f:
    	...
    	newText = newText.replace('app_load_balancer_ip', app_name+"_load_balancer_ip")
...
TERRAFORM_CLUSTER_STATE_FILENAME = "terraform.tfstate"


if __name__ == "__main__":
    #Retrieve AKS Cluster authentification informations from infrastructure Managed Entity configuration variables.
    Orchestration.update_asynchronous_task_details(*async_update_list, f'Retrieve AKS Cluster authentification details ...')
    ...
    kubernetes_cluster_name = _get_configuration_variable(device, 'kubernetes_cluster_name')
    ...
    Orchestration.update_asynchronous_task_details(*async_update_list, f'Retrieve AKS Cluster authentification details ...OK')
    ...
    cluster_terraform_remote_state = cluster_tf_work_directory +"/"+ TERRAFORM_CLUSTER_STATE_FILENAME
    set_tfconfig_variable_value(ftvars_filename, 'cluster_terraform_remote_state', cluster_terraform_remote_state, context)
    
    f1 = open(ftvars_filename, 'a+')
    f2 = open(cluster_tf_work_directory+"/"+TERRAFORM_TFVARS_FILENAME, 'r')
    f1.write(f2.read())
    f1.close()
    f2.close()
